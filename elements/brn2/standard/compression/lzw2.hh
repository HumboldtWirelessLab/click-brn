#ifndef LEMPEL_ZIV_WELCH_2_HH
#define LEMPEL_ZIV_WELCH_2_HH
...
class LZWCodec2
{
 ...
 private:
  ...
  void reset_tables();
  unsigned int input_code_ORIG(unsigned char *input, int *pos);
  int find_match(unsigned int hash_prefix, unsigned int hash_character);
  unsigned char *decode_string(unsigned char *buffer,unsigned int code);
  unsigned int mask(const int n_bits);
  unsigned int input_code(unsigned char *input, int *pos, int inputlen, const int n_bits);
  void output_code(unsigned char *output, int *pos, unsigned int code, const int n_bits);
