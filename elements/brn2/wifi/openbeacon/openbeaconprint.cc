...
OpenBeaconPrint::simple_action(Packet *p)
{
	...
	char cpower[10], cchannel[10], crate[10];
	...
	if(crh->rate<2) sprintf(crate, "%d", crh->rate-1); else sprintf(crate, "current");
	
	
	click_chatter("%s# Power: %s, Rate: %s, Channel: %s, DEST: %s, SRC: %s",_label.c_str(), cpower, crate, cchannel, dmac_sa.c_str(), smac_sa.c_str());
	return p;
}
