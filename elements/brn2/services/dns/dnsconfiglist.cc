...
BRN2DNSConfigList::BRN2DNSConfigList()
{
}
...
int
BRN2DNSConfigList::configure(Vector<String> &conf, ErrorHandler *errh)
{
  _debug = false;
...
BRN2DNSConfigList::DHCPSubnet *
BRN2DNSConfigList::getSubnet(int index) {
...
BRN2DNSConfigList::DHCPSubnet *
BRN2DNSConfigList::getSubnetByVlanID(int id) {
  BRN2DNSConfigList::DHCPSubnet *sn;
...
static String
BRN2DNSConfigList_read_param(Element *e, void *thunk)
{
  BRN2DNSConfigList *snl = (BRN2DNSConfigList *)e;
  switch ((uintptr_t) thunk) {
    ...
    case H_READ: {
      ...
      for ( int i = 0; i < snl->_subnet_list.size(); i++ ) {
        BRN2DNSConfigList::DHCPSubnet sn = snl->_subnet_list[i];
...
static int
BRN2DNSConfigList_write_param(const String &in_s, Element *e, void *vparam,
                                 ErrorHandler *errh)
{
  BRN2DNSConfigList *f = (BRN2DNSConfigList *)e;
  ...
  switch((intptr_t)vparam) {
    ...
    case H_INSERT: {
      ...
      f->_subnet_list.push_back(BRN2DNSConfigList::DHCPSubnet(_me, _net_address, _subnet_mask, _broadcast_address,
...
void
BRN2DNSConfigList::add_handlers()
{
  add_read_handler("debug", BRN2DNSConfigList_read_param, (void *) H_DEBUG);
  add_read_handler("info", BRN2DNSConfigList_read_param, (void *) H_READ);

  add_write_handler("debug", BRN2DNSConfigList_write_param, (void *) H_DEBUG);
  add_write_handler("insert", BRN2DNSConfigList_write_param, (void *) H_INSERT);
}

#include <click/vector.cc>
template class Vector<BRN2DNSConfigList::DHCPSubnet>;

CLICK_ENDDECLS
EXPORT_ELEMENT(BRN2DNSConfigList)
