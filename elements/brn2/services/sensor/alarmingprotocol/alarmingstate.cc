...
void
AlarmingState::get_incomlete_forward_nodes(int max_fraction, int max_retries, int type, Vector<AlarmNode*> *nodes)
{
  for( int an_i = _alarm_nodes.size()-1; an_i >= 0; an_i--) {
    ...
    if ( an->_type == type ) {
      for( int ai_i = an->_info.size()-1; (ai_i >= 0); ai_i--) {
        ...
        if ( (ai->_fract_fwd < max_fraction) && (ai->_retries < max_retries) ) {
          nodes->push_back(an);
          break;
        }
      }
