...
/*
 * CompressedAlarmingRetransmit.{cc,hh}
 */
...
CompressedAlarmingRetransmit::CompressedAlarmingRetransmit():
  _timer(this),
  _alarm_active(false),
...
int
CompressedAlarmingRetransmit::configure(Vector<String> &conf, ErrorHandler* errh)
{
...
int
CompressedAlarmingRetransmit::initialize(ErrorHandler *)
{
...
void
CompressedAlarmingRetransmit::run_timer(Timer */*t*/)
{
//  click_chatter("Timer");
  if ( _alarm_active ) {
...
void
CompressedAlarmingRetransmit::retransmit_active(bool active)
{
...
void
CompressedAlarmingRetransmit::retransmit_alarm()
{
  ...
  Vector<CompressedAlarmingState::AlarmNode*> nodes;
...
//    click_chatter("get node for type %d",alarm);
    ...
    for ( int an_i = (nodes.size()-1); an_i >= 0; an_i-- ) {
      CompressedAlarmingState::AlarmNode *an = nodes[an_i];

      for ( int ai_i = an->_info.size() - 1; ai_i >= 0; ai_i-- ) {
        CompressedAlarmingState::AlarmInfo *ai = &(an->_info[ai_i]);

        if ( p == NULL ) p = CompressedAlarmingProtocol::new_alarming_packet(alarm);

        p = CompressedAlarmingProtocol::add_node(p, &(an->_ea), START_TTL + ai->_hops, ai->_id);
        ai->_retries++;
      }
    }
...
static int
CompressedAlarmingRetransmit_write_param(const String &in_s, Element *e, void *vparam,
                             ErrorHandler *errh)
{
  CompressedAlarmingRetransmit *as = (CompressedAlarmingRetransmit *)e;
...
void
CompressedAlarmingRetransmit::add_handlers()
{
  BRNElement::add_handlers();

  add_write_handler("active", CompressedAlarmingRetransmit_write_param,(void *) H_RETRANSMIT_ACTIVE);
}

CLICK_ENDDECLS
EXPORT_ELEMENT(CompressedAlarmingRetransmit)
