...
/*
 * CompressedAlarmingAggregation.{cc,hh}
 */
...
#include "compressedalarmingaggregation.hh"
#include "compressedalarmingstate.hh"
#include "compressedalarmingprotocol.hh"

CLICK_DECLS

CompressedAlarmingAggregation::CompressedAlarmingAggregation():
  _delay_queue(NULL),
  _alarm_ret(NULL),
  _no_agg(0),
  _no_pkts(0)
...
int
CompressedAlarmingAggregation::configure(Vector<String> &conf, ErrorHandler* errh)
{
...
int
CompressedAlarmingAggregation::initialize(ErrorHandler *)
{
  return 0;
}

Packet *
CompressedAlarmingAggregation::pull(int /*port*/)
{
  bool has_next = true;
  Packet *p, *p_agg;
  Packet *p_out = NULL;
...
/*      if ( _delay_queue != NULL ) {
        ...
        int count_info_in_p = CompressedAlarmingProtocol::get_count_nodes(p);  //next packet
...
//        click_chatter("Got %d in packets",count_info_in_p);
        for ( int i = 0; i < count_info_in_p; i++ ) {                //for each data, check whether it is already in the out packet
          struct compressed_alarming_node *an_agg = CompressedAlarmingProtocol::get_node_struct(p_agg, i);

          EtherAddress n_ea = EtherAddress(an_agg->node_ea);
          struct compressed_alarming_node *an_p_out = CompressedAlarmingProtocol::get_node_struct(p_out, &n_ea, an_agg->id);
...
//            click_chatter("New node");
            ...
            p_out = CompressedAlarmingProtocol::add_node(p_out, &n_ea, new_ttl, an_agg->id);
          }
        }
...
void
CompressedAlarmingAggregation::add_handlers()
{
  BRNElement::add_handlers();
}

CLICK_ENDDECLS
EXPORT_ELEMENT(CompressedAlarmingAggregation)
