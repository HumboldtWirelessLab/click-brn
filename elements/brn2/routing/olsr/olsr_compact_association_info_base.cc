...
//			printf("inserting %s: first if branch: matches %s\n", ippair._from.unparse_with_mask(ippair._to).c_str(), prev->_from.unparse_with_mask(prev->_to).c_str());
			curr = _compactSet->insert(prev, OLSRIPPair(curr->_from & IPAddress::make_prefix(prev->_to.mask_to_prefix_len()-1), IPAddress::make_prefix(prev->_to.mask_to_prefix_len()-1)));
			next = curr+1;	
...
//			printf("inserting %s: second if branch: matches %s\n", ippair._from.unparse_with_mask(ippair._to).c_str(), next->_from.unparse_with_mask(next->_to).c_str());
			curr = _compactSet->insert(curr, OLSRIPPair(curr->_from & IPAddress::make_prefix(next->_to.mask_to_prefix_len()-1), IPAddress::make_prefix(next->_to.mask_to_prefix_len()-1)));
			next = curr+1;		
			next = _compactSet->erase(next);	
			next = curr+1;	
			next = _compactSet->erase(next);			
