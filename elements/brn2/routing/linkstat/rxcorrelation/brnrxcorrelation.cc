...
BrnRXCorrelation::configure(Vector<String> &conf, ErrorHandler* errh)
{
  ...
  if (cp_va_kparse(conf, this, errh,
      "LINKSTAT", cpkP+cpkM, cpEtherAddress, &_linkstat,
      "DEBUG", cpkP+cpkM, cpInteger, &_debug,
      "NOTE_LP", cpkP+cpkM, cpInteger, &_note_lp,
      cpEnd) < 0)
       return -1;
...
/*
int BrnRXCorrelation::add_recv_linkprobe(EtherAddress &_addr, unsigned int lp_id)
{
  int i;
  ...
  {
    NeighbourInfo *_new_ci = new NeighbourInfo(_addr, _note_lp, _linkprobe_id - 1);  // create new one
 ...
 */
 /*
int BrnRXCorrelation::add_mention_linkprobe(EtherAddress &node, unsigned int lp_id)
{
  int i;
  ...
  if ( i == _cand.size() )
  {
    NeighbourInfo *_new_ci = new NeighbourInfo(node, _note_lp, _linkprobe_id - 1);  // create new one
...
unsigned int BrnRXCorrelation::writeCsPayloadToLinkProbe(uint8_t *ptr, unsigned int payload_size)
{
  uint16_t size;
  uint32_t val_32;
  uint16_t val_16;
...
void BrnRXCorrelation::readCsPayloadFromLinkProbe(EtherAddress &source, uint8_t *ptr, unsigned int *//*payload_size*//*)
{
  uint16_t size;
  uint32_t val_32;
  uint16_t val_16;
    ...
    if ( cand_addr == _me )
    {
      for ( cand_i = 0; cand_i < _cand.size(); cand_i++ )
      {
        if ( _cand[cand_i]->_addr == source )
        {
          ...
          else click_chatter("BrnRXCorrelation::readCsPayloadFromLinkProbe: I thought he knows more");
          break;
        }
      }
      ...
      {
        NeighbourInfo *_new_ci = new NeighbourInfo(source, _note_lp, _linkprobe_id - 1);  // create new one
...
uint8_t BrnRXCorrelation::has_lp(Vector<unsigned int> &lp_list, int lp_id)
{
...
int BrnRXCorrelation::calculatedRecvPER(NeighbourInfo *cand)
{
  return calculatedRecvPER(cand,_note_lp);
}

int BrnRXCorrelation::calculatedRecvPER(NeighbourInfo *cand, int number_of_calc_lp)
{
...
int BrnRXCorrelation::calculatedPER(NeighbourInfo *cand)
{
  return calculatedPER(cand,_note_lp);
}

int BrnRXCorrelation::calculatedPER(NeighbourInfo *cand, int number_of_calc_lp)
{
 ...
 * @return 
 *//*
int BrnRXCorrelation::calculatedCombinedPER(Vector<NeighbourInfo*> &sel_cand, int number_of_calc_lp)
{
...
*/
/*
int BrnRXCorrelation::getAndTestPossibleCS(Vector<NeighbourInfo*> *can,int cssize, int *per, int ac_pos, Vector<NeighbourInfo*> *best_can, int number_of_calc_lp)
{
  int ac_per;
...
int BrnRXCorrelation::getCandidateSet(Vector<NeighbourInfo*> *can, int cssize, int *per)
{
  Vector<NeighbourInfo*> cs_try;
...
*/
/*
int BrnRXCorrelation::getBestCandidates(NeighbourInfo **cand, int number_of_cand)
{
  for( int i = 0; i < _cand.size(); i++ ) _cand[i]->send_per = calculatedPER(_cand[i]);

  click_qsort(_cand.begin(), _cand.size(), sizeof(NeighbourInfo *), send_per_sorter);
...
String BrnRXCorrelation::printNeighbourInfo()
{
  StringAccum sa;
  NeighbourInfo *_ci;

  Vector<NeighbourInfo*> sel_cand;
  int per,j;

  click_chatter("PrintInfo");

  sa << "NeighbourInfo ( Node: " << _me.unparse() << " )\n";
...
String BrnRXCorrelation::printPerInfo()
{
  StringAccum sa;
  //NeighbourInfo *_ci;
...
String BrnRXCorrelation::printRecvPerInfo()
{
  StringAccum sa;
 // NeighbourInfo *_ci;
...
{
  return ((BrnRXCorrelation*)e)->printPerInfo();
}

static String
read_handler_recv_per(Element *e, void *)
{
  return ((BrnRXCorrelation*)e)->printRecvPerInfo();
}
*/
static int 
write_handler_debug(const String &in_s, Element *e, void *,ErrorHandler *errh)
{
  BrnRXCorrelation *brncansec = (BrnRXCorrelation*)e;
