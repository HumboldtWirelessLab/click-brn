...
//  -> Print("RouteQuery")
  -> querier[0]
//  -> Print("DSR: querie")
  -> [2]output;                                             // rreq packets (broadcast)
...
device_wifi[1] -> EtherDecap() -> brn_clf;
...
dsr[1] -> Print("DSR[1]-ap-out") -> BRN2EtherEncap() /*-> Print("Dsr-ether-out") */-> [0]device_wifi;
dsr[2] -> EtherEncap(0x8086, deviceaddress, FF:FF:FF:FF:FF:FF) -> [0]device_wifi;
...
//BRN2PacketSource(500, 10000, 1000, 10,1,15)
//-> EtherEncap(0x8080, deviceaddress, 00:0f:00:00:00:00 )
//-> [1]device_wifi;

Idle
-> sf::BRN2SimpleFlow(SRCADDRESS deviceaddress, DSTADDRESS 00:0f:00:00:04:00, RATE 250, SIZE 1000, MODE 0, DURATION 100000, ACTIVE 0)
-> BRN2EtherEncap()
//-> Print("Packet Ether")
-> [0]dsr;
