...
BRNDS::push(int port, Packet *p_in)
{
  ...
  EtherAddress dst_anno = p_in->dst_ether_anno();
  ...
  if (brn_dsr->dsr_type == BRN_DSR_RREQ) { // broadcast packet
    ...
    if (WritablePacket *p_wlan0 = p_in->push(14)) {
      ...
      //set device anno
      p_wlan0->set_udevice_anno(_me->getWlan0DeviceName().c_str());
      p_vlan0->set_udevice_anno(_me->getVlan0DeviceName().c_str());
      p_vlan1->set_udevice_anno(_me->getVlan1DeviceName().c_str());
