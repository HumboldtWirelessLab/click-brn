...
#ifndef BRNLINKSTATELEMENT_HH
#define BRNLINKSTATELEMENT_HH

/*
=c
BRNLinkStat([I<KEYWORDS>])
...
=item STOP
Unsigned integer. Milliseconds period after which to stop sending probes
Default is 0 which means never.
...
class BrnRateSize {
 public:
  ...
  inline bool
  operator==(BrnRateSize other)
  {
    return (other._rate == _rate && other._size == _size);
  }
};

class BRNLinkStat : public Element {

public:

  // Packet formats:

  // BRNLinkStat packets have a link_probe header immediately following
  ...
  struct link_probe {
    ...
    unsigned int _dht_payload_size; // number of bytes used to inform neighbors about some dht stuff
...
public:
  ...
  void send_probe();
  static void static_send_hook(Timer *, void *e) { ((BRNLinkStat *) e)->send_probe_hook(); }
...
//  unsigned long _log_id;
 ...
 public:
  ...
  const char *class_name() const		{ return "BRNLinkStat"; }
