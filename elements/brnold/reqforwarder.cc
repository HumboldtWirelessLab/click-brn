...
// 
      ...
      if (old_frv && ! _route_querier->metric_preferable(this_metric, old_frv->best_metric)) { 
        ...
        BRN_DEBUG("* forwarding this RREQ %s %s", device.c_str(), p_in->udevice_anno().c_str());
...
RequestForwarder::forward_rreq(Packet *p_in)
{
  ...
  p->set_dst_ether_anno(EtherAddress((const unsigned char *)"\xff\xff\xff\xff\xff\xff"));

  // copy device anno
  p->set_udevice_anno(device.c_str());
