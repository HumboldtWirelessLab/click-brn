...
InfrastructureClient::configure(Vector<String> &conf, ErrorHandler* errh)
{
  ...
  if (!_beaconscanner || !_beaconscanner->cast("BeaconScanner")) 
...
InfrastructureClient::find_best_ap()
{
  ...
  for (BeaconScanner::APIter iter = _beaconscanner->_waps.begin(); iter.live(); iter++) {
    BeaconScanner::wap ap = iter.value();
