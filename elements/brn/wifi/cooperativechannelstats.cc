...
CooperativeChannelStats::CooperativeChannelStats() :
        _cst(NULL), _msg_timer(this), _interval(0), _add_neighbours(false)
{
...
int CooperativeChannelStats::configure(Vector<String> &conf, ErrorHandler* errh)
{
    ...
    int ret = cp_va_kparse(conf, this, errh, "CHANNELSTATS", cpkP + cpkM, cpElement, &_cst, "INTERVAL", cpkP, cpInteger, &_interval, "NEIGHBOURS", cpkP, cpBool,
            &_add_neighbours, "DEBUG", cpkP, cpInteger, &_debug, cpEnd );
...
{
    WritablePacket *p = Packet::make(128, NULL, sizeof(struct cooperative_channel_stats_header) + sizeof(neighbour_airtime_stats) * ccsh.no_neighbours, 32);
...
//TODO "check stettings of brn_headers"
...
void CooperativeChannelStats::push(int, Packet *p)
{
    ...
    if(ccsh.flags > 0)
    {
        ...
        for(uint8_t i = 0; i < ccsh.no_neighbours; i++)
        {
            ...
            BRN_INFO("RECEIVED: Address %s: bytes: %d, packets: %d, duration: %d, avg_rssi: %d", temp_ea.unparse().c_str(), temp_nats->_byte_count,
                    temp_nats->_pkt_count, temp_nats->_duration, temp_nats->_avg_rssi);
