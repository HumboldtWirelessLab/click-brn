...
AssocList::configure(Vector<String> &conf, ErrorHandler* errh)
{
  ...
  if (!_id && !_id->cast("NodeIdentity"))
    return errh->error("NodeIdentity element is not a NodeIdentity");
...
int
AssocList::initialize(ErrorHandler *errh)
{
...
bool
AssocList::is_associated(const EtherAddress& v) const
{
...
bool 
AssocList::is_roaming(const EtherAddress& v) const
{
  ClientInfo* pClient = _client_list->findp(v);
  if (NULL == pClient)
    return false;
...
AssocList::ClientInfo *
AssocList::get_entry(const EtherAddress& v)
{
...
String
AssocList::get_device_name(const EtherAddress& v) const
{
  ClientInfo* pClient = _client_list->findp(v);
...
String
AssocList::get_ssid(const EtherAddress& v) const
{
  ClientInfo* pClient = _client_list->findp(v);
  //BRN_DEBUG(" _client_list->findp(*v) = 0x%x", pClient);
...
bool
AssocList::insert(  EtherAddress    eth, 
                    String          dev_name,
                    String          ssid, 
                    ...
                    uint8_t         seq_no /*= SEQ_NO_INF*/)
{
  ...
  client_info->set_ap(*_id->getMyWirelessAddress());
...
EtherAddress 
AssocList::get_ap(const EtherAddress& client) const
{
...
AssocList::PacketList 
AssocList::get_buffered_packets(
  EtherAddress ether_dst)
{
  assert (ether_dst);
  ClientInfo* pClient = get_entry(ether_dst);
...
bool 
AssocList::remove(EtherAddress eth)
{
  if (NULL == _client_list->findp(eth))
    return (false);

  _link_table->update_both_links( *_id->getMyWirelessAddress(), eth, 
