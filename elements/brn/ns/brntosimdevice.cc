...
#include "brnfromsimdevice.hh"
#include "brntosimdevice.hh"
...
int
BRNToSimDevice::configure(Vector<String> &conf, ErrorHandler *errh)
{
  String encap_type;
...
int
BRNToSimDevice::initialize(ErrorHandler *errh)
{
  _fd = -1;
  if (!_ifname)
    return errh->error("interface not set");
...
void
BRNToSimDevice::uninitialize()
{
  _task.unschedule();
}

void
BRNToSimDevice::send_packet(Packet *p)
{
  Router* myrouter = router();
  int retval;
...
void
BRNToSimDevice::push(int, Packet *p)
{
  assert(p->length() >= 14);
...
CLICK_ENDDECLS
ELEMENT_REQUIRES(BRNFromSimDevice ns)
EXPORT_ELEMENT(BRNToSimDevice)
