#ifndef CLICK_BRNTODEVICE_USERLEVEL_HH
#define CLICK_BRNTODEVICE_USERLEVEL_HH
#include <click/element.hh>
#include <click/string.hh>
#include <click/task.hh>
#include <click/timer.hh>
#include <click/notifier.hh>
#include "elements/brn2/userlevel/brnfromdevice.hh"
CLICK_DECLS

/*
 * =title BrnToDevice.u
 * =c
 * BrnToDevice(DEVNAME [, I<KEYWORDS>])
 * =s netdevices
 * sends packets to network device (user-level)
 * =d
 *
 * This manual page describes the user-level version of the BrnToDevice element.
 * For the Linux kernel module element, read the BrnToDevice(n) manual page.
 *
 ...
 * =n
 *
 * Packets sent via BrnToDevice should already have a link-level
 * header prepended. This means that ARP processing,
 * for example, must already have been done.
 *
 * The L<BrnFromDevice(n)> element's OUTBOUND keyword argument determines whether
 * BrnFromDevice receives packets sent by a BrnToDevice element for the same
 * device.
 *
 ...
 *
 * =a
 * BrnFromDevice.u, FromDump, ToDump, KernelTun, BrnToDevice(n) */
...
class BrnToDevice : public Element { public:

  BrnToDevice();
  ~BrnToDevice();

  const char *class_name() const		{ return "BrnToDevice"; }
