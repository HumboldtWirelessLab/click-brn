#ifndef CLICK_BRNFROMDEVICE_USERLEVEL_HH
#define CLICK_BRNFROMDEVICE_USERLEVEL_HH
...
# endif
void BrnFromDevice_get_packet(u_char*, const struct pcap_pkthdr*, const u_char*);
}
#endif
CLICK_DECLS

/*
=title BrnFromDevice.u

=c

BrnFromDevice(DEVNAME [, I<keywords> SNIFFER, PROMISC, SNAPLEN, FORCE_IP, CAPTURE, BPF_FILTER, OUTBOUND, HEADROOM])
...
This manual page describes the user-level version of the BrnFromDevice
element. For the Linux kernel module element, read the BrnFromDevice(n) manual
page.

Reads packets from the kernel that were received on the network controller
named DEVNAME.

User-level BrnFromDevice behaves like a packet sniffer by default.  Packets
emitted by BrnFromDevice are also received and processed by the kernel.  Thus, it
...
using the KernelFilter element or BrnFromDevice's SNIFFER false argument.

Under Linux, a BrnFromDevice element will not receive packets sent by a
...
Boolean.  Specifies whether BrnFromDevice should run in sniffer mode.  In
non-sniffer mode, BrnFromDevice installs KernelFilter filtering rules to block
the kernel from handling any packets arriving on device DEVNAME.  Default is
true (sniffer mode).

=item PROMISC

Boolean.  BrnFromDevice puts the device in promiscuous mode if PROMISC is true.
...
Word.  Defines the capture method BrnFromDevice will use to read packets from the
...
BrnFromDevice sets packets' extra length annotations as appropriate.
...
constraints, before BrnFromDevice could get them. This may be an integer; the
...
=a ToDevice.u, FromDump, ToDump, KernelFilter, BrnFromDevice(n) */

class BrnFromDevice : public Element { public:

    BrnFromDevice();
    ~BrnFromDevice();

    const char *class_name() const	{ return "BrnFromDevice"; }
  ...
  private:
    ...
    friend void BrnFromDevice_get_packet(u_char*, const struct pcap_pkthdr*,
				      const u_char*);
