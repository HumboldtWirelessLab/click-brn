...
int
BrnFlooding::configure(Vector<String> &conf, ErrorHandler* errh)
{
  int max_jitter;
...
int
BrnFlooding::initialize(ErrorHandler *)
{
  bcast_id = 0;
...
void
BrnFlooding::push( int port, Packet *packet )  
{
  click_chatter("BrnFlooding: PUSH\n");
  ...
  { 
    click_chatter("BrnFlooding: PUSH vom Client\n");
  ...
  {
    click_chatter("BrnFlooding: PUSH von BRN\n");
...
long
BrnFlooding::diff_in_ms(timeval t1, timeval t2)
{
  long s, us, ms;
...
unsigned int
BrnFlooding::get_min_jitter_in_queue()
{
...
template class Vector<BrnFlooding::BrnBroadcast>;
template class Vector<BrnFlooding::BufferedPacket>;

CLICK_ENDDECLS
EXPORT_ELEMENT(BrnFlooding)
