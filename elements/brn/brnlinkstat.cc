...
BRNLinkStat::configure(Vector<String> &conf, ErrorHandler* errh)
{
  ...
  int res = cp_va_parse(conf, this, errh,
      ...
      "BRNAVGCNT", cpElement, "BrnAvgCnt", &_packetCnt,
			...
			"STOP", cpUnsigned, "stop sending probes (msecs)", &end_probes_msec,
  //                      "LOG_INTERVALL", cpInteger, "Logging Interval (in msecs)", &_log_interval,
  //                      "LOG_FILENAME", cpFilename, "log filename", &_log_filename,
			cpEnd);

  if (end_probes_msec) {
  	_end_probes = Timestamp::now();
  	_end_probes += Timestamp::make_msec(end_probes_msec);
  }
...
BRNLinkStat::send_probe_hook() 
{
  ...
  if (!_end_probes || _end_probes > _next)
...
//      BRN_DEBUG("BRNLinkStat: got and send Linkprop-Info: %s", buffer);
  ...
  while (ptr < end && num_entries < _neighbors.size()) {
    ...
    if (!probe) {
      ...
      if ((uint32_t) probe->_ether > (uint32_t) *(_me->getMyWirelessAddress())) {
        entry->_seq = lp->_seq;
      }
...
BRNLinkStat::simple_action(Packet *p)
{
  ...
  while (ptr < end 
    ...
    int seq = entry->_seq;
    if (neighbor == ether && 
        ((uint32_t) neighbor > (uint32_t) *(_me->getMyWirelessAddress()))) {
      seq = now.tv_sec;
    }
