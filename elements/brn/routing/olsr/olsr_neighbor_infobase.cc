...
{// as described in the RFC, chapter 8.3.1 MPR Computation
	...
	HashMap<OLSRIPPair, void*> *linkSet=_linkInfoBase->get_link_set();
			...
			if (neigh)
			{
				...
				{								//a twohopborset for this
					...
					if ((IP_Vector_ptr=coverage.findp(main_address)))	//all nodes reachable from this neighbor are twohop neighbors
						for (int i=0;i<IP_Vector_ptr->size();i++)
						{
							...
							twohop_of_interface.insert (OLSRIPPair(main_address,N_twohop_addr),_twohopSet->find(OLSRIPPair(main_address,N_twohop_addr)));		//include this in the new created hasmap
							bool insert=false;
				...
				else 				//allmost the same as above, but adding the it to an existing list
				{
					...
					if ((IP_Vector_ptr=coverage.findp(main_address)))	//all nodes reachable from this neighbor are twohop neighbors
					{
						...
						{
							IPAddress N_twohop_addr=(*(IP_Vector_ptr))[i];
							twohop_of_interface_ptr->insert (OLSRIPPair(main_address,N_twohop_addr),_twohopSet->find(OLSRIPPair(main_address,N_twohop_addr)));		//include this in the existing TwoHopSet for this interface
