...
EewsAggregation::EewsAggregation():
  _delay_queue(NULL),
  _alarm_ret(NULL),
  _no_agg(0),
  _no_pkts(0)
...
int
EewsAggregation::configure(Vector<String> &conf, ErrorHandler* errh)
{
  if (cp_va_kparse(conf, this, errh,
      "EEWSSTATE", cpkP+cpkM, cpElement, &_as,
      "DELAYQUEUE", cpkP, cpElement, &_delay_queue,
      "EEWSRETRANSMIT", cpkP, cpElement, &_alarm_ret,
...
int
EewsAggregation::initialize(ErrorHandler *)
{
  return 0;
}

Packet *
EewsAggregation::pull(int /*port*/)
{
  bool has_next = true;
  Packet *p, *p_agg;
  Packet *p_out = NULL;
...
/*      if ( _delay_queue != NULL ) {
        ...
        int count_info_in_p = EewsProtocol::get_count_nodes(p);  //next packet
...
//        click_chatter("Got %d in packets",count_info_in_p);
        for ( int i = 0; i < count_info_in_p; i++ ) {                //for each data, check whether it is already in the out packet
          struct eews_node *an_agg = EewsProtocol::get_node_struct(p_agg, i);

          EtherAddress n_ea = EtherAddress(an_agg->node_ea);
          struct eews_node *an_p_out = EewsProtocol::get_node_struct(p_out, &n_ea, an_agg->id);
...
//            click_chatter("New node");
            ...
            p_out = EewsProtocol::add_node(p_out, &n_ea, new_ttl, an_agg->id);
          }
        }
...
//            click_chatter("Update");
 ...
 //         click_chatter("New node");
  ...
  if ( p_out != NULL ) {
    p_out = BRNProtocol::add_brn_header(p_out, BRN_PORT_EEWSPROTOCOL, BRN_PORT_EEWSPROTOCOL, p_out_ttl, 0);
    _no_pkts++;
  } else {
...
void
EewsAggregation::add_handlers()
{
  BRNElement::add_handlers();
}

CLICK_ENDDECLS
EXPORT_ELEMENT(EewsAggregation)
