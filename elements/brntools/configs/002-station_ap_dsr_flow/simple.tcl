...
for {set i 2} {$i < $nodecount } {incr i} {
    #
    ...
    # script.
    #
[$node_($i) entry] loadclick "simple.click"
}

[$node_(0) entry] loadclick "simple_client.click"
[$node_(1) entry] loadclick "simple_ap_dsr.click"
...
for {set i 0} {$i < $nodecount} {incr i} {
    $node_($i) set X_ [expr $i % 8 * 50 ]
    $node_($i) set Y_ [expr $i / 8 * 50 ]
    $node_($i) set Z_ 0
