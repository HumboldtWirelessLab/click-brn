...
int
DSRDecap::initialize(ErrorHandler *errh)
{
 ...
 */
void
DSRDecap::extract_request_route(const Packet *p_in, int *ref_metric, RouteQuerierRoute &route)
{
...
#ifdef CLICK_NS
  EtherAddress *my_rec_addr = _me->getMyWirelessAddress(); // ethernet use wlan only in NS2
#else
  EtherAddress *my_rec_addr = _me->getEthernetAddress(device); // ethernet addr of the interface the packet is coming from
#endif
 ...
 */
void
DSRDecap::extract_reply_route(const Packet *p, RouteQuerierRoute &route)
{
 ...
 */
void
DSRDecap::extract_source_route(const Packet *p_in, RouteQuerierRoute &route)
{
